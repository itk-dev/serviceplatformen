 ...
 * - definition: urn:oio:sagdok:organisationtilstandliste:ADD:2.0.0 | urn:oio:sagdok:organisationenhedtilstandliste:ADD:2.0.0 | urn:oio:sagdok:organisationfunktiontilstandliste:ADD:2.0.0 | urn:oio:sagdok:adressetilstandliste:ADD:2.0.0 |
 * urn:oio:sagdok:persontilstandliste:ADD:2.0.0 | urn:oio:sagdok:brugertilstandliste:ADD:2.0.0 | urn:oio:sagdok:interessefaellesskabtilstandliste:ADD:2.0.0 | urn:oio:sagdok:itsystemtilstandliste:ADD:2.0.0 |
 * urn:oio:sagdok:myndighedtilstandliste:ADD:2.0.0 | urn:oio:sagdok:virksomhedtilstandliste:ADD:2.0.0
 * @subpackage Structs
 */
class TilstandListeType extends AbstractStructBase
{
     ...
     * @var \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType[]
     */
     ...
     * @param \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType[] $gyldighed
     * @param \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\LokalUdvidelseType $lokalUdvidelse
     */
    public function __construct(?array $gyldighed = null, ?\ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\LokalUdvidelseType $lokalUdvidelse = null)
    {
        $this
    ...
    /**
     * Get Gyldighed value
     * @return \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType[]
     */
    ...
    public static function validateGyldighedForArrayConstraintFromSetGyldighed(?array $values = []): string
    {
        ...
        foreach ($values as $tilstandListeTypeGyldighedItem) {
            ...
            if (!$tilstandListeTypeGyldighedItem instanceof \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Gyldighed property can only contain items of type \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType[] $gyldighed
     * @return \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\TilstandListeType
     */
     ...
     * @throws InvalidArgumentException
     * @param \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType $item
     * @return \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\TilstandListeType
     */
    public function addToGyldighed(\ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType) {
            throw new InvalidArgumentException(sprintf('The Gyldighed property can only contain items of type \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\GyldighedType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->Gyldighed[] = $item;
