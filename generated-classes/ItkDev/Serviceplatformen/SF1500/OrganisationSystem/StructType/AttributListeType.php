 ...
 * - definition: urn:oio:sagdok:organisationattributliste:ADD:2.0.0 | urn:oio:sagdok:organisationenhedattributliste:ADD:2.0.0 | urn:oio:sagdok:organisationfunktionattributliste:ADD:2.0.0 | urn:oio:sagdok:adresseattributliste:ADD:2.0.0 |
 * urn:oio:sagdok:personattributliste:ADD:2.0.0 | urn:oio:sagdok:brugerattributliste:ADD:2.0.0 | urn:oio:sagdok:interessefaellesskabattributliste:ADD:2.0.0 | urn:oio:sagdok:itsystemattributliste:ADD:2.0.0 |
 * urn:oio:sagdok:myndighedattributliste:ADD:2.0.0 | urn:oio:sagdok:virksomhedattributliste:ADD:2.0.0
 * @subpackage Structs
 */
class AttributListeType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * - ref: org:Egenskab | orgenhed:Egenskab | orgfunk:Egenskab | adresse:Egenskab | person:Egenskab | bruger:Egenskab | intfs:Egenskab | itsystem:Egenskab | myndighed:Egenskab | virksomhed:Egenskab
     * @var \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType[]
     */
     ...
     * @param \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType[] $egenskab
     * @param \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\LokalUdvidelseType $lokalUdvidelse
     */
    public function __construct(?array $egenskab = null, ?\ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\LokalUdvidelseType $lokalUdvidelse = null)
    {
        $this
    ...
    public static function validateEgenskabForArrayConstraintFromSetEgenskab(?array $values = []): string
    {
        ...
        foreach ($values as $attributListeTypeEgenskabItem) {
            ...
            if (!$attributListeTypeEgenskabItem instanceof \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Egenskab property can only contain items of type \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType[] $egenskab
     * @return \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\AttributListeType
     */
     ...
     * @throws InvalidArgumentException
     * @param \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType $item
     * @return \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\AttributListeType
     */
    public function addToEgenskab(\ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType) {
            throw new InvalidArgumentException(sprintf('The Egenskab property can only contain items of type \ItkDev\Serviceplatformen\SF1500\OrganisationSystem\StructType\EgenskabType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->Egenskab[] = $item;
