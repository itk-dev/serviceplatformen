...
class GyldighedType extends AbstractStructBase
{
     ...
     */
    public function __construct(?\ItkDev\Serviceplatformen\SF1500\Bruger\StructType\VirkningType $virkning = null, ?string $gyldighedStatusKode = null)
    {
        $this
     ...
     * @uses \ItkDev\Serviceplatformen\SF1500\Bruger\EnumType\GyldighedStatusKodeType::valueIsValid()
     * @uses \ItkDev\Serviceplatformen\SF1500\Bruger\EnumType\GyldighedStatusKodeType::getValidValues()
    ...
    public function setGyldighedStatusKode(?string $gyldighedStatusKode = null): self
    {
        ...
        if (!\ItkDev\Serviceplatformen\SF1500\Bruger\EnumType\GyldighedStatusKodeType::valueIsValid($gyldighedStatusKode)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \ItkDev\Serviceplatformen\SF1500\Bruger\EnumType\GyldighedStatusKodeType', is_array($gyldighedStatusKode) ? implode(', ', $gyldighedStatusKode) : var_export($gyldighedStatusKode, true), implode(', ', \ItkDev\Serviceplatformen\SF1500\Bruger\EnumType\GyldighedStatusKodeType::getValidValues())), __LINE__);
        }
