 ...
 * - definition: urn:oio:sagdok:facettilstandliste:ADD:2.0.0
 * @subpackage Structs
 */
class TilstandListeType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * - ref: facet:PubliceretStatus
     * @var \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType[]
     */
    protected ?array $PubliceretStatus = null;
    /**
     * Constructor method for TilstandListeType
     * @uses TilstandListeType::setPubliceretStatus()
     * @param \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType[] $publiceretStatus
     */
     ...
     * @return \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType[]
     */
    ...
    public static function validatePubliceretStatusForArrayConstraintFromSetPubliceretStatus(?array $values = []): string
    {
        ...
        foreach ($values as $tilstandListeTypePubliceretStatusItem) {
            ...
            if (!$tilstandListeTypePubliceretStatusItem instanceof \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PubliceretStatus property can only contain items of type \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType[] $publiceretStatus
     * @return \ItkDev\Serviceplatformen\SF1500\Facet\StructType\TilstandListeType
     */
     ...
     * @throws InvalidArgumentException
     * @param \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType $item
     * @return \ItkDev\Serviceplatformen\SF1500\Facet\StructType\TilstandListeType
     */
    public function addToPubliceretStatus(\ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType) {
            throw new InvalidArgumentException(sprintf('The PubliceretStatus property can only contain items of type \ItkDev\Serviceplatformen\SF1500\Facet\StructType\PubliceretStatusType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
