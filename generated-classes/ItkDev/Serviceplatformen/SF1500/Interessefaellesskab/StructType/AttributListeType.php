 ...
 * - definition: urn:oio:sagdok:interessefaellesskabattributliste:ADD:2.0.0
 * @subpackage Structs
 */
class AttributListeType extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: intfs:Egenskab
     * @var \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType[]
     */
     ...
     * @param \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType[] $egenskab
     * @param \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\LokalUdvidelseType $lokalUdvidelse
     */
    public function __construct(?array $egenskab = null, ?\ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\LokalUdvidelseType $lokalUdvidelse = null)
    {
        $this
    ...
    /**
     * Get Egenskab value
     * @return \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType[]
     */
    ...
    public static function validateEgenskabForArrayConstraintFromSetEgenskab(?array $values = []): string
    {
        ...
        foreach ($values as $attributListeTypeEgenskabItem) {
            ...
            if (!$attributListeTypeEgenskabItem instanceof \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Egenskab property can only contain items of type \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType[] $egenskab
     * @return \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\AttributListeType
     */
     ...
     * @throws InvalidArgumentException
     * @param \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType $item
     * @return \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\AttributListeType
     */
    public function addToEgenskab(\ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType) {
            throw new InvalidArgumentException(sprintf('The Egenskab property can only contain items of type \ItkDev\Serviceplatformen\SF1500\Interessefaellesskab\StructType\EgenskabType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->Egenskab[] = $item;
