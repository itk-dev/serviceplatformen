...
class GyldighedType extends AbstractStructBase
{
     ...
     */
    public function __construct(?\ItkDev\Serviceplatformen\SF1500\OrganisationFunktion\StructType\VirkningType $virkning = null, ?string $gyldighedStatusKode = null)
    {
        $this
     ...
     * @uses \ItkDev\Serviceplatformen\SF1500\OrganisationFunktion\EnumType\GyldighedStatusKodeType::valueIsValid()
     * @uses \ItkDev\Serviceplatformen\SF1500\OrganisationFunktion\EnumType\GyldighedStatusKodeType::getValidValues()
    ...
    public function setGyldighedStatusKode(?string $gyldighedStatusKode = null): self
    {
        ...
        if (!\ItkDev\Serviceplatformen\SF1500\OrganisationFunktion\EnumType\GyldighedStatusKodeType::valueIsValid($gyldighedStatusKode)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \ItkDev\Serviceplatformen\SF1500\OrganisationFunktion\EnumType\GyldighedStatusKodeType', is_array($gyldighedStatusKode) ? implode(', ', $gyldighedStatusKode) : var_export($gyldighedStatusKode, true), implode(', ', \ItkDev\Serviceplatformen\SF1500\OrganisationFunktion\EnumType\GyldighedStatusKodeType::getValidValues())), __LINE__);
        }
