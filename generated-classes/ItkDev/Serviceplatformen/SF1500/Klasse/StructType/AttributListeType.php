 ...
 * - definition: urn:oio:sagdok:klasseattributliste:ADD:2.0.0
 * @subpackage Structs
 */
class AttributListeType extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: klasse:Egenskab
     * @var \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType[]
     */
    protected ?array $Egenskab = null;
    /**
     * Constructor method for AttributListeType
     * @uses AttributListeType::setEgenskab()
     * @param \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType[] $egenskab
     */
    ...
    public static function validateEgenskabForArrayConstraintFromSetEgenskab(?array $values = []): string
    {
        ...
        foreach ($values as $attributListeTypeEgenskabItem) {
            ...
            if (!$attributListeTypeEgenskabItem instanceof \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Egenskab property can only contain items of type \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType[] $egenskab
     * @return \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\AttributListeType
     */
     ...
     * @throws InvalidArgumentException
     * @param \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType $item
     * @return \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\AttributListeType
     */
    public function addToEgenskab(\ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType) {
            throw new InvalidArgumentException(sprintf('The Egenskab property can only contain items of type \ItkDev\Serviceplatformen\SF1500\Klasse\StructType\EgenskabType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->Egenskab[] = $item;
