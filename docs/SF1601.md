...
```sh
docker run --interactive --tty --rm --volume ${PWD}:/app itkdev/php7.4-fpm:latest composer reinstall goetas-webservices/xsd2php
docker run --interactive --tty --rm --volume ${PWD}:/app itkdev/php7.4-fpm:latest bin/xsd2php
```

Check everything before `git commit`'ing the `lib` folder!
